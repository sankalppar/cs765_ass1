...
class Peer:
    ...
    def generate_block(self, parent):
        time = np.random.exponential(scale = I/hashing_power)
...
def init_Peers(n, z0, z1, balance_scale):
    ...
    gen_block = Block(gen_block_id, None, gen_txns, [], None)
...
if __name__ == "__main__":
    ...
    for i in range(n);
        time, blk = peers[i].generate_block()
        data = {"blk": blk}
        print("Time:", time)
    ...
    while(not event_queue.empty()):
        ...
        if(type=="BlkGenerated"):
            ...
            valid_blk = False
            for i in range(len(peer[creator_index].leaves)):
                if peer[creator_index].leaves[i][0] == blk.parent:
                    peers[creator_index].leaves[i][0] = blk
                    peers[creator_index].leaves[i][1] += 1
                    valid_blk = True
                    break

            #Check if block still valid
            if not valid_blk:
                last_blk = sorted(leaves, key=lambda tup: tup[1], reverse=True)
                time, new_blk = peers[creator_index].generate_block(last_blk[0])
                data = {'blk': new_blk}
