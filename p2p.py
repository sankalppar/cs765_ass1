...
'''
Class for storing information related to peer related information. Stores n, boolean
on whether the peer is slow or has low cpu power, balances of all peers, set of connected
peers, transaction pool, hashing power, unique id etc.
'''
...
class Peer:
    ...
    '''
    Functions associated to a peer. Includes function to add a neighbor, generate
    a transaction, generate a coinbase transaction and genrating a block
    '''
    ...
    def generate_coinbase(self):
        txn_id = '0' + self.id + format(self.coinbase_generated, '06d')
...
'''
Class for transaction, Stores unique txn ID, ID of payer, ID of receiver and
transaction amount.
'''
...
'''
Class for block. Stores block Id, parent, list of transactions, size of block,
ID of creator, balance after block is added to its parent. Also contains function
to add a deep copy of block as a child(deep copy is needed as each node maintains
a separate copy of the block)
'''
...
'''
Function to initialize peers. The balances of peers are initialised randomly and
the initial balances are assigned to them by the genesis block. We also select
random peers and assign them to be slow or low cpu. We use the fraction z1 to
determine the hashing power for both high and low cpu peers.
'''
...
'''
Function to initialise the connected graph. Selects random degree from 4 to 8
for each peer and then chooses random nodes to join. Based on whether the graph
is connected or not it generates the graph again.
'''
...
'''
Sets the value of latency for the links randomly.
'''
...
if __name__ == "__main__":
    ...
    while(not event_queue.empty()):
        ...
        if(type=="BlkGenerated"):
            ...
            #Update balance and transaction pool stored in peer
        ...
        if(type=="BlkReceived"):
            ...
            #Determine if this blockchain becomes longest and update accordingly
    ...
    #Saving block related info into a file for debugging, commented for normal functioning
